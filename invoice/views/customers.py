...
def customer(request, customer_id):
    ...
    return render(request, 'invoice/customer.html', context=context)
...
def update_customer(request, customer_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            region = request.POST.get('state', 'None')
            country = request.POST.get('zip', 'None')
            Customer.objects.filter(pk=customer_id).update(name=name, city=city, region=region, country=country)
